...
void LCD_PIN_Init(void){
	...
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIOAEN;  //A
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIOBEN;  //B
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIOCEN;  //C
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIODEN;  //D
...
void LCD_Configure(void){
	...
	// Configure DUTY[2:0] bits of the LCD_CR and set the duty to 1/4
	LCD->CR &= ~(LCD_CR_DUTY);  //clear the targeted bits
	LCD->CR |= LCD_CR_DUTY_0;   //set the 2 LSB high
	LCD->CR |= LCD_CR_DUTY_1;
	
	// Configure the CC[2:0] bits of LCR_FCR and set the contrast value to the maximum
	LCD->FCR &= ~(LCD_FCR_CC);  //clear the targeted bits
	LCD->FCR |= LCD_FCR_CC_0;   //set all 3 bits high
	LCD->FCR |= LCD_FCR_CC_1; 
	LCD->FCR |= LCD_FCR_CC_2; 
	
	//Configure the PON[2:0] bits of LCD_FCR and set the pulse on period to the maximum
	LCD->FCR &= ~(LCD_FCR_PON);  //clear the targeted bits
	LCD->FCR |= LCD_FCR_PON_0;   //set all 3 bits high
	LCD->FCR |= LCD_FCR_PON_1; 
	LCD->FCR |= LCD_FCR_PON_2; 
	
	//Disable the MUX_SEG segment of LCD_CR
	LCD->CR &= ~(LCD_CR_MUX_SEG);  //disable it by setting the targeted bit to low.
	
	//Select internal voltage as LCD voltage source
	LCD->CR &= ~(LCD_CR_VSEL); //set the bit to low
	
	//Wait until the FCRSF flag of LCD_SR is set.
	//Check the FCRSF bit and wait until it's high to continue running the program
	//while ((LCD->CR & LCD_SR_FCRSF) == 0); /////////////////////////////////////////////////
	
	//Enable the LCD by setting LCDEN bit of LCD_CR
	LCD->CR &= ~LCD_CR_LCDEN;
	LCD->CR |= LCD_CR_LCDEN; //set the bit high
...
//input: pointer to the first char of a string. We add an offset to 
